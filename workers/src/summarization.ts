...
const isRemovableEmptySectionTitle = (title: string): boolean => {
	...
	return removablePrefixes.some((prefix) => normalizedTitle.startsWith(prefix));
};
...
const removeEmptySections = (summary: string): string => {
	...
	const filtered = sections.filter((section) => {
		...
		if (!isRemovableEmptySectionTitle(section.title)) {
			return true;
		}
